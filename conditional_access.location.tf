...
resource "azuread_conditional_access_policy" "cal001" {
  ...
  display_name = "CAL001-All: Block specified locations for All users when Browser and Modern Auth Clients-v1.0"
...
resource "azuread_conditional_access_policy" "cal002" {
  ...
  display_name = "CAL002-All: Require MFA registration from trusted locations only for All users when Browser and Modern Auth Clients-v1.0"
...
resource "azuread_group" "cal003_exclude" {
  count = var.cal003_create == true && var.trusted_locations != null ? 1 : 0 # Policy can't be created if no trusted location are defined in var.trusted_locations, therefore no group needed either.
...
resource "azuread_conditional_access_policy" "cal003" {
  count = var.cal003_create == true && var.trusted_locations != null ? 1 : 0 # Policy can't be created if no trusted location are defined in var.trusted_locations.

  display_name = "CAL003-All: Block all locations except trusted locations for Privileged users when Browser and Modern Auth Clients-v1.0"
...
data "azuread_users" "cal004_include" {
  count = var.cal004_included_user_upns != null ? 1 : 0 # Don't get data about users if var.cal004_included_user_upns if not specified.

  user_principal_names = var.cal004_included_user_upns
}

### Create exclude group
resource "azuread_group" "cal004_exclude" {
  count = var.cal004_create == true && var.cal004_included_user_upns != null && var.trusted_locations != null ? 1 : 0 # Policy can't be created if no UPNs are defined in var.cal004_included_user_upns or no trusted locations are defined in var.trusted_locations, therefore no group needed either.
...
resource "azuread_conditional_access_policy" "cal004" {
  count = var.cal004_create == true && var.cal004_included_user_upns != null && var.trusted_locations != null ? 1 : 0 # Policy can't be created if no UPNs are defined in var.cal004_included_user_upns or no trusted locations are defined in var.trusted_locations.

  display_name = "CAL004-All: Block all locations except trusted locations for Selected users when Browser and Modern Auth Clients-v1.0"
  state        = var.reporting_only_for_all_policies == true ? "enabledForReportingButNotEnforced" : var.cal004_state

  conditions {
    users {
      included_users  = data.azuread_users.cal004_include[0].object_ids
